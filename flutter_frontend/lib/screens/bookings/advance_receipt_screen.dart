...
class _AdvanceReceiptScreenState extends ConsumerState<AdvanceReceiptScreen> {
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(existing ? (shown.receiptNumber ?? 'Receipt') : 'Advance receipt'),
          actions: [
            Padding(
                      ...
                      if (existing) ...[
                        ...
                        SizedBox(
                          ...
                          child: PdfPreview(
                            ...
                            build: (format) => AdvanceReceiptPdf.build(shown),
                            canChangePageFormat: false,
                            canChangeOrientation: false,
                            canDebug: false,
                            // The print/share bar is the row of icon buttons
                            // below the document, not this widget's own —
                            // without this, the desk saw the same two
                            // actions offered twice on one screen.
                            useActions: false,
                            pdfFileName: '${shown.receiptNumber ?? shown.id}.pdf',
                          ),
                        ),
                        Padding(
  ...
  Widget _header(AdvanceReceipt? shown, bool loaded, bool existing) {
    ...
    return Column(
      ...
      children: [
        Text(
          '${b.guestName ?? 'Guest'} · stay total '
          '${formatPrice(_stayTotal)}'
          '${_alreadyHeld > 0 ? ' · ${formatPrice(_alreadyHeld)} advance held' : ''}'
          '${_remaining > 0 ? ' · ${formatPrice(_remaining)} still to pay' : ' · paid in full'}',
          ...
          children: [
            ...
            if (shown != null && !shown.isVoid)
              TextButton(
                ...
                child: Text(_voiding ? 'Voiding…' : 'Void this receipt'),
              ),
          ],
        ),
      ],
  ...
  Widget _takeForm() {
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Advance received now', style: Theme.of(context).textTheme.titleMedium),
          ...
          Row(
            children: [
              const Expanded(
                child: Text('Amount', style: TextStyle(color: AppTheme.text, fontSize: 13)),
              ),
              Text(
                formatPrice(_amount),
                style: const TextStyle(
          ...
          if (_takeError != null) ...[
            const SizedBox(height: AppTheme.s8),
            Text(_takeError!, style: const TextStyle(color: AppTheme.danger, fontSize: 12)),
          ],
          const SizedBox(height: AppTheme.s16),
          Row(
            children: [
              Expanded(
                child: NeuButton(
                  onPressed: _taking ? null : () => setState(() => _showTakeForm = false),
              ...
              Expanded(
                child: NeuButton(
                  ...
                  child: Text(_taking ? 'Recording…' : 'Record & print receipt'),
                ),
              ),
            ],
          ),
...
class _ReceiptChip extends StatelessWidget {
  ...
  const _ReceiptChip({required this.receipt, required this.active, required this.onTap});
