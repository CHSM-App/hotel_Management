...
-- ---------------------------------------------------------------------------
-- Expense Management module (migrations 076-077)
-- ---------------------------------------------------------------------------
--
-- dbo.vendors above is now a shared payee directory used by both Assets and
-- Expenses — no schema change needed for that, just no longer Assets-owned.

-- A real table rather than a fixed enum, same reasoning as asset_categories.
-- Left empty by default — a category exists once it's typed or picked from a
-- suggestion list in the expense form and used to save an expense.
...
CREATE TABLE dbo.expenses (
    ...
    -- dbo.assets.bill_document. Access goes through the authenticated
...
IF EXISTS (SELECT 1 FROM dbo.roles WHERE lodge_id IS NULL AND role_key = 'OWNER'
           AND permissions = '["rooms.manage","bookings.manage","billing.manage","guests.view","reports.view","staff.manage","food.manage","orders.manage","events.manage","assets.manage"]')
UPDATE dbo.roles
